...
class Trainer(object, metaclass=abc.ABCMeta):
    def __init__(self, args):
        ...
        self.trlog['test_max_acc'] = 0.0
        self.trlog['test_max_acc_epoch'] = 0
        self.trlog['test_max_acc_interval'] = 0.0
        self.trlog['test_max_auc'] = 0.0
        self.trlog['test_max_auc_epoch'] = 0
        self.trlog['test_max_auc_interval'] = 0.0
        self.trlog['test_max_f1'] = 0.0
        self.trlog['test_max_f1_epoch'] = 0
        self.trlog['test_max_f1_interval'] = 0.0
    ...
    def try_evaluate(self, epoch):
        ...
        if self.train_epoch % args.eval_interval == 0:
            vl, va, vap, vauc, vaucp = self.evaluate(self.val_loader)
            self.logger.add_scalar('val_loss', float(vl), self.train_epoch)
            self.logger.add_scalar('val_acc', float(va),  self.train_epoch)
            self.logger.add_scalar('val_auc', float(vauc),  self.train_epoch)

            print('epoch {}, val, loss={:.4f} acc={:.4f}+{:.4f} auc={:.4f}+{:.4f}'.format(epoch, vl, va, vap, vauc, vaucp))
            ...
            if va >= self.trlog['max_auc']:
                self.trlog['max_auc'] = vauc
                self.trlog['max_auc_interval'] = vaucp
                self.trlog['max_auc_epoch'] = self.train_epoch
                self.save_model('max_auc')

    def try_logging(self, tl1, tl2, ta, tauc, tg=None):
        args = self.args
        if self.train_step % args.log_interval == 0:
            print('epoch {}, train {:06g}/{:06g}, total loss={:.4f}, loss={:.4f} acc={:.4f} auc={:.4f}, lr={:.4g}'
                  .format(self.train_epoch,
                          self.train_step,
                          self.max_steps,
                          tl1.item(), tl2.item(), ta.item(), tauc.item(),
            ...
            self.logger.add_scalar('train_auc',  tauc.item(), self.train_step)
            if tg is not None:
