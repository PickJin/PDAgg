...
def cal_auc2(logits, logits_open):
    logits_all = torch.cat([logits, logits_open], dim=0)
    logits_all = F.softmax(logits_all, dim=1)
                
    output_max = torch.max(logits_all, dim=1)[0].cpu().detach().numpy()
    open_label_binary = np.concatenate((np.ones(logits.shape[0]),np.zeros(logits_open.shape[0])))
    auc = metrics.roc_auc_score(open_label_binary, output_max)

    return auc

def cal_auc(logits, logits_open):
    logits_all = torch.cat([logits, logits_open], dim=0)
    #logits_all = F.softmax(logits_all, dim=1)
    #print('logits_all', logits_all.shape, logits_all)
                
    output_max = torch.max(logits_all, dim=1)[0].cpu().detach().numpy()
    #print('output_max', output_max.shape, output_max)
    open_label_binary = np.concatenate((np.ones(logits.shape[0]),np.zeros(logits_open.shape[0])))
    #print('open_label_binary', open_label_binary.shape, open_label_binary)
    auc = metrics.roc_auc_score(open_label_binary, output_max)

    return auc

def cal_auc_temp(logits, logits_open):
    ...
    open_label_binary = np.concatenate((np.ones(logits.shape[0]),np.zeros(logits_open.shape[0])))
    #print('open_label_binary', open_label_binary.shape, open_label_binary)
    auc = metrics.roc_auc_score(open_label_binary, logits_all)
...
def get_command_line_parser():
    ...
    #parser.add_argument('--init_weights', default='/disk/8T/jinj/FSL/FSOR/tmp/released/miniImageNet_pre.pth', type=str)   ##mini的预训练模型
    parser.add_argument('--init_weights', default='/disk/8T/jinj/FSL/FSOR/tmp/released/tieredImageNet_pre.pth', type=str)
    #parser.add_argument('--init_weights', default='/disk/8T/jinj/FSL/FSOR/tmp/TANE_self/max_meta_5shot.pth', type=str)
    #parser.add_argument('--init_weights', default='/disk/8T/jinj/FSL/FSOR/tmp/Res12-pre.pth', type=str)
    #parser.add_argument('--init_weights', default='/disk/8T/jinj/FSL/FSOR/tmp/tired-Res12-pre.pth', type=str)
    ...
    parser.add_argument('--save_dir', type=str, default='/disk/8T/jinj/FSL/FSOR/tmp4/FEAT_checkpoints')
    parser.add_argument('--rotangle', type=int, default=10,help='rotation angle of the weak augmentation')
    #parser.add_argument('--data_root', type=str, default='/disk/8T/jinj/FSL/dataset/mini_imagenet/miniImageNet', help='path to data root')
    parser.add_argument('--data_root', type=str, default='/disk/8T/jinj/FSL/dataset/tiered-imagenet-wordemb', help='path to data root')
    #parser.add_argument('--data_dir', type=str, default='/disk/8T/datasets', help='path to data root')
    parser.add_argument('--data_dir', type=str, default='/disk/8T/datasets', help='path to data root')
    ...
    parser.add_argument('--data_comp', action='store_true', default=True)
    return parser
